 ...
 */
export function Chart({ title, spec, data, tooltip }: ChartProps) {
  const ref = useRef<HTMLDivElement>(null);
  const [dimensions, setDimensions] = useState<Dimensions>();

  useEffect(() => {
    const resizeObserver = new ResizeObserver((entries) => {
      if (Boolean(entries) && Boolean(entries.length)) {
  ...
  return (
    <div className="mb-4 w-full">
      {Boolean(title) && (
        <h3 className="z-10 mb-2 text-sm font-medium">{title}</h3>
      )}
      <div ref={ref} className={twMerge("relative flex-grow")}>
        {Boolean(dimensions) && Boolean(data) && Boolean(spec) && (
          <VegaLite
            className="w-full"
            spec={spec}
            data={data}
